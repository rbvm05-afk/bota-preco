...
const newMaterial = (): MaterialItem => ({ id: crypto.randomUUID(), name: "", paid: 0, packageAmount: 0, packageUnit: "g", usedAmount: 0, usedUnit: "g" });
...
export function PricingWizard({ mode }: { mode: "rapidin" | "completao" }) {
  ...
  const [input, setInput] = useState<PricingInput>({
    productName: "", yieldAmount: 1, materials: [newMaterial()], workHours: 1, hourlyRate: 15,
    packagingItems: [], packagingPerUnit: 0, extraCosts: 0, wastePercent: isComplete ? 5 : 3,
    salesFeePercent: 0, desiredMargin: isComplete ? 35 : 30,
  });
  ...
  const back = () => step > 1 ? setStep((current) => current - 1) : router.push("/");
  const finish = () => {
  ...
  const completed = (id: string) => questions.findIndex((q) => q.id === id) < step - 1;
  const showSummary = completed("materials");

  return (
    <AppShell compact>
      <Progress current={step} total={totalSteps} labels={questions.map((item) => item.label)} showTotal={!isComplete} />
      {showSummary && <LiveSummary input={input} showLabor={completed("hourlyRate")} showPackaging={completed("packaging")} showExtras={completed("extras")} />}
      ...
      <section className="surface animate-rise rounded-[2.2rem] p-5 sm:p-8">
        <div className="mb-7 flex items-center justify-between gap-3 border-b border-[var(--border)] pb-5">
          <span className="rounded-full bg-[var(--green-soft)] px-3 py-1.5 text-xs font-black uppercase tracking-[.14em] text-[var(--green)]">{isComplete ? "🎯 Completão" : "⚡ Rapidin"}</span>
          <span className="text-right text-xs font-bold text-[var(--muted)]">💾 A conta entra no histórico quando terminar</span>
        </div>

        <QuestionRenderer question={currentQuestion} input={input} profile={profile} update={update} />

        {isComplete && step === 6 && <AdSlot placement="completao-mid-flow" className="mt-8" />}
        <div className="mt-9 flex flex-col-reverse gap-3 border-t border-[var(--border)] pt-6 sm:flex-row">
          <button type="button" onClick={back} className="rounded-2xl border border-[var(--border)] bg-white px-5 py-4 font-black transition hover:-translate-y-0.5 hover:bg-[#f4f1eb] sm:w-36">← Voltar</button>
          {step === totalSteps ? <PrimaryButton onClick={finish}>✨ Mostrar meu preço</PrimaryButton> : <PrimaryButton onClick={next} disabled={currentQuestion.id === "product" && !input.productName.trim()}>Continuar →</PrimaryButton>}
        </div>
      </section>
    </AppShell>
  );
...
function QuestionRenderer({ question, input, profile, update }: { question: WizardQuestion; input: PricingInput; profile: ReturnType<typeof resolveProfile>; update: <K extends keyof PricingInput>(field: K, value: PricingInput[K]) => void }) {
  ...
  return (
    <QuestionBlock eyebrow={question.eyebrow} title={title} text={text}>
      ...
      {question.kind === "margin" && <div className="rounded-2xl border border-green-200 bg-[var(--green-soft)] p-4 text-sm font-bold leading-6 text-[var(--green-deep)]">✅ Pronto. Depois desta resposta, o Bota junta tudo e mostra de onde saiu o preço.</div>}
    </QuestionBlock>
  );
}

function QuestionBlock({ eyebrow, title, text, children }: { eyebrow: string; title: string; text: string; children: React.ReactNode }) {
  return <div className="space-y-6"><div><p className="mb-2 font-black text-[var(--green)]">{eyebrow}</p><h1 className="text-3xl font-black leading-tight tracking-[-.035em] sm:text-4xl">{title}</h1><p className="mt-3 leading-7 text-[var(--muted)]">{text}</p></div>{children}</div>;
}
